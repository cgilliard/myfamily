...
MyTest(args, test_args_param) {
	...
	args_param_build(&ap, "threads", "number of threads to execute", "t", true, false, NULL);
...
MyTest(args, test_sub_command) {
	...
	args_param_build(&ap1, "threads", "number of threads to execute", "t", true, false, "1");
	args_param_build(&ap2, "port", "tcp/ip port to bind to", "p", true, true, "8080");
	args_param_build(&ap3, "debug", "print debug info", "d", false, false, NULL);

	cr_assert(!sub_command_build(&sc, "node", "start the full node", 1, 2, "arg doc"));
