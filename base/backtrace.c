...
int backtrace_add_entry(Backtrace* ptr, BacktraceEntry* entry)
{
    ...
    if (ptr->count == 0) {
        ...
        BacktraceEntry* tmp = realloc(ptr->rows, sizeof(BacktraceEntry) * (ptr->count + 1));
        if (tmp == NULL) {
            ret = -1;
        } else {
            ptr->rows = tmp;
    ...
    if (!ret) {
        ptr->rows[ptr->count].function_name = malloc(sizeof(char) * (strlen(entry->function_name) + 1));
        strcpy(ptr->rows[ptr->count].function_name, entry->function_name);

        ptr->rows[ptr->count].bin_name = malloc(sizeof(char) * (strlen(entry->bin_name) + 1));
        strcpy(ptr->rows[ptr->count].bin_name, entry->bin_name);

        ptr->rows[ptr->count].address = malloc(sizeof(char) * (strlen(entry->address) + 1));
        strcpy(ptr->rows[ptr->count].address, entry->address);

        ptr->rows[ptr->count].file_path = malloc(sizeof(char) * (strlen(entry->file_path) + 1));
