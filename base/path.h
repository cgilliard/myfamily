...
typedef struct PathNc {
	const u8 *buf;
	u64 len;
} PathNc;
#define INIT_PATH {.name = NULL, .len = 0}

void path_cleanup(PathNc *ptr);

#define Path PathNc __attribute__((warn_unused_result, cleanup(path_cleanup)))
