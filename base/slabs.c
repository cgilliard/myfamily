		...
		for (int i = 0;
			...
			for (int j = 0;
				...
				for (int k = 0; k < PAGE_SIZE / sizeof(unsigned char *) &&
								impl->data[i][j][k];
					 k++)
					munmap((unsigned char *)impl->data[i][j][k], PAGE_SIZE);
				munmap((unsigned char *)impl->data[i][j], PAGE_SIZE);
			}
			munmap((unsigned char *)impl->data[i], PAGE_SIZE);
		}
		munmap((unsigned char *)impl->data, PAGE_SIZE);
	}
}
