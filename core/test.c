...
Test(orbtree_rand) {
	...
	int64 *values = map(1 + (sizeof(int64) * size) / PAGE_SIZE);

	for (int i = 0; i < size; i++) {
		arr[i] = slab_allocator_allocate(&sa);
		MyObject *obj = arr[i].data;
		int64 v = 0;
		cpsrng_rand_int64(&v);
		values[i] = v;
		obj->value = v;
	...
	unmap(arr, 1 + (sizeof(Slab) * size / PAGE_SIZE));
	unmap(values, 1 + (sizeof(int64) * size / PAGE_SIZE));
}
...
Test(orbtree_perf) {
	...
	int64 *keys = map(1 + (size * sizeof(int64) / PAGE_SIZE));
	...
	for (int x = 0; x < count; x++) {
		for (int64 i = 0; i < size; i++) {
			Slab slab = slab_allocator_allocate(&sa);
			MyLong *obj = slab.data;
			obj->slab = slab;
			obj->x = keys[i];
			obj->y = i;
		...
		for (int64 i = 0; i < size; i++) {
			...
			slab_allocator_free(&sa, &ref->slab);
		}
	}
	...
	slab_allocator_cleanup(&sa);
	unmap(keys, 1 + (size * sizeof(int64) / PAGE_SIZE));
}
