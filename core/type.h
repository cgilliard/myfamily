...
#define ReturnObj(obj)                                                                             \
	({                                                                                             \
		Obj _ret__ = obj;                                                                          \
		Obj_mark_consumed(&obj);                                                                   \
		return _ret__;                                                                             \
	})
#define ReturnObjAndConsumeSelf(obj)                                                               \
	({                                                                                             \
		Obj _ret__ = obj;                                                                          \
		Obj_mark_consumed(&obj);                                                                   \
		Obj_cleanup($());                                                                          \
...
#define $Context(value, type, ...)                                                                 \
	__VA_OPT__(((const type *)(value)->ptr.data)->__VA_ARGS__)                                     \
	__VA_OPT__(NONE)                                                                               \
	(value)

#define $ContextVar(value, type, ...)                                                              \
	__VA_OPT__(((type *)(value)->ptr.data)->__VA_ARGS__)                                           \
	__VA_OPT__(NONE)                                                                               \
	(value)
...
#define PROCESS_WHERE_INNER__(...) __VA_OPT__(FIRST_STRINGIFY __VA_ARGS__)
...
#define BUILD_OBJECTS_IMPL_(name, struct_type, inner, vtable_name, ...)                            \
	__VA_OPT__(NONE)                                                                               \
	(BUILD_OBJECTS_IMPL__(name, struct_type, inner, vtable_name))                                  \
		__VA_OPT__(BUILD_OBJECT_GENERIC(name, struct_type, inner, vtable_name))
#define BUILD_OBJECTS_IMPL(name, struct_type, inner, ...)                                          \
	BUILD_OBJECTS_IMPL_(name, struct_type, inner, __VA_ARGS__)
#define BUILD_OBJECTS_IMPL__(name, struct_type, inner, vtable_name)                                \
	({                                                                                             \
		...
		for (u64 i = 0; name##_generic_name_arr != NULL && name##_generic_name_arr[i] != NULL;     \
			 i++) {                                                                                \
...
#define BUILD_OBJECTS____(name, struct_type, inner, ...)                                           \
	__VA_OPT__(BUILD_OBJECTS_IMPL(name, struct_type, inner, __VA_ARGS__))
...
#define TypeImpl(name, static_value, ...)                                                          \
	...
	typedef struct name {                                                                          \
		EXPAND(EXPAND_ALL __VA_OPT__(PAREN) __VA_OPT__(PAREN_END)                                  \
				   __VA_OPT__(NONE)(char _dummy__;))                                               \
		__VA_OPT__(FOR_EACH(CALL_FIRST_TWO, , (;), __VA_ARGS__);)                                  \
	} name;                                                                                        \
	static_value u64 name##_size() {                                                               \
		return sizeof(name);                                                                       \
	...
	static_value void __attribute__((constructor)) __add_impls_##name##_vtable() {                 \
		...
		VtableEntry build_internal = {"build_internal", name##_build_internal};                    \
		vtable_add_entry(&name##_Vtable__, build_internal);                                        \
		VtableEntry drop_internal = {"drop_internal", name##_drop_internal};                       \
		...
		if (((*((Obj *)dst)).vtable != NULL &&                                                     \
			...
			if ((*((Obj *)dst)).vtable->is_trait_specifier) {                                      \
				...
				for (u64 i = 0; i < (*((Obj *)dst)).vtable->trait_len; i++) {                      \
					if (!Implements((*((Obj *)src)),                                               \
									(*((Obj *)dst)).vtable->trait_entries[i].trait_name))          \
...
#define PROC_FN_SIGNATURE(...) __VA_OPT__(FOR_EACH(UNWRAP_PARAM_TYPE_NAME, , (, ), __VA_ARGS__))
...
#define PROCESS_FN_CALL(...) __VA_OPT__(FOR_EACH_INNER(CALL_SECOND, none, (, ), __VA_ARGS__))

#define OBJECT_ONLY(i, value, ...)                                                                 \
	_Generic((value), Obj *: Obj_check_param((const Obj *)value), default: Obj_check_param(NULL))
...
#define EXPAND3(x) x
#define EXPAND2(x) x
#define PROC_BOTH_PROTO__(impl_type, param_type, param_name, ignore1, ...)                         \
	__VA_OPT__(const impl_type##Config *)                                                          \
	EXPAND3 __VA_OPT__(PAREN) __VA_OPT__(PAREN_END) __VA_OPT__(NONE)(const Obj *)
#define PROC_BOTH_PROTO_(impl_type, param_type, param_name, ...)                                   \
	__VA_OPT__(PROC_BOTH_PROTO__(impl_type, param_type, param_name, __VA_ARGS__))                  \
	EXPAND2 __VA_OPT__(NONE)(param_type param_name)
#define PROC_BOTH_PROTO(...) PROC_BOTH_PROTO_(__VA_ARGS__)
#define CALL_BOTH_PROTO(x, y) PROC_BOTH_PROTO(x, EXPAND_ALL y)
#define PROCESS_FN_SIG_PROTO(impl_type, ...)                                                       \
	__VA_OPT__(FOR_EACH_INNER(CALL_BOTH_PROTO, impl_type, (, ), __VA_ARGS__))
...
#define CHECK_SELF_IMPL__(self, type, var_name, ...)                                               \
	__VA_OPT__(CHECK_SELF_IMPL___(self, type, var_name, __VA_ARGS__))
...
#define CHECK_SELF_PARAMS(self, ...)                                                               \
	__VA_OPT__(FOR_EACH_INNER(CHECK_SELF_IMPL, self, (;), __VA_ARGS__))
	...
	static return_type fn_name(mutability() Obj *self __VA_OPT__(, )                               \
		...
		if (self->flags & OBJECT_FLAGS_CONSUMED) {                                                 \
			if (self->vtable == NULL)                                                              \
				...
				panic("Runtime error: Obj [%s@%" PRIu64 "] has already been consumed!",            \
					  self->vtable->name, self->ptr.id);                                           \
		}                                                                                          \
		return_type (*impl)(__VA_OPT__(PROCESS_FN_SIG(__VA_ARGS__))) = find_fn(self, #fn_name);    \
		if (!impl)                                                                                 \
...
#define DEFAULT_IMPL__(T, default_impl_fn, mutability, return_type, fn_name, ...)                  \
	...
	static void __attribute__((constructor)) add_##fn_name##_##T##_vtable() {                      \
		VtableEntry next = {#fn_name, default_impl_fn};                                            \
		vtable_add_entry(&T##_Vtable__, next);                                                     \
	}

#define CHECK_SUPER_(type, trait)                                                                  \
	static void __attribute__((constructor)) check_super_##type##_##trait() {                      \
...
#define DEFAULT_IMPL_(x, ...) MULTI_SWITCH2_INNER(SUPER_PROC, DEFAULT_IMPL__, x, __VA_ARGS__)
#define DEFAULT_IMPL(T, ...) DEFAULT_IMPL_(T, EXPAND_ALL __VA_ARGS__)

#define PROC_TRAIT_IMPL_FN_DEFAULT__(default_fn, mutability, return_type, fn_name, ...)            \
	static return_type fn_name(mutability() Obj *self __VA_OPT__(, )                               \
								   __VA_OPT__(PROCESS_FN_SIG(__VA_ARGS__))) {                      \
		UNEXPECTED_FLAGS(self->flags);                                                             \
		if (self->flags & OBJECT_FLAGS_CONSUMED) {                                                 \
...
#define PROC_TRAIT_IMPL_FN_DEFAULT_(x, ...)                                                        \
	MULTI_SWITCH2(SUPER_TRAIT_CONFIG, PROC_TRAIT_IMPL_FN_DEFAULT__, x, __VA_ARGS__)
#define PROC_TRAIT_IMPL_FN_DEFAULT(...) PROC_TRAIT_IMPL_FN_DEFAULT_(EXPAND_ALL __VA_ARGS__)

#define PROC_TRAIT_IMPL_FN(x, ...)                                                                 \
	MULTI_SWITCH(PROC_TRAIT_IMPL_FN_DEFAULT, PROC_TRAIT_IMPL_FN_, x, __VA_ARGS__)
...
#define PROC_TRAIT_STATEMENT_IMPL_EXP_(impl_type, mutability, return_type, fn_name, ...)           \
	return_type impl_type##_##fn_name(__VA_OPT__(PROCESS_FN_SIG_PROTO(impl_type, __VA_ARGS__)));   \
	static void __attribute__((constructor)) __required_add__##impl_type##_##fn_name() {           \
...
#define PROC_TRAIT_STATEMENT_IMPL_EXP(x, y, ...)                                                   \
	MULTI_SWITCH2(DEFAULT_IMPL, PROC_TRAIT_STATEMENT_IMPL_EXP_, x, y, __VA_ARGS__)
#define PROC_TRAIT_STATEMENT_IMPL(...) PROC_TRAIT_STATEMENT_IMPL_EXP(__VA_ARGS__)
#define PROC_TRAIT_STATEMENT(arg, x) PROC_TRAIT_STATEMENT_IMPL(arg, EXPAND_ALL x)
#define PROC_IMPL(name, ...) __VA_OPT__(FOR_EACH(PROC_TRAIT_STATEMENT, name, (), __VA_ARGS__))
...
#define PROC_IMPL_(name, trait_name, ...)                                                          \
	...
	static void __attribute__((constructor)) vtable_add_trait_impl_##name##_##trait_name() {       \
...
#define SET_OFFSET_OF_IMPL(ptr, structure, name, value, ...)                                       \
	__VA_OPT__(do {                                                                                \
		if (IS_OBJECT_TYPE(((structure *)0)->name)) {                                              \
			...
			*((typeof(((structure *)0)->name) *)((char *)ptr + offsetof(structure, name))) =       \
				value;                                                                             \
			ENABLE_WARNING_WCONST                                                                  \
		}                                                                                          \
	} while (0);)                                                                                  \
	EXPAND(EXPAND(EXPAND_ALL EXPAND(__VA_OPT__(NONE)(PROC_WITH_OBJ(ptr, structure, name, value))   \
										__VA_OPT__(()))))
