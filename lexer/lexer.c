...
char *lexer_read_line(Lexer *l) {
	...
	while (true) {
		if (ret == NULL) {
			...
			char *tmp = realloc(ret, sizeof(char) * LEXER_BUF_SIZE * (1 + count));
			if (tmp == NULL) {
				free(ret);
				return NULL;
			}
...
int lexer_init(Lexer *l, char *file) {
	...
	l->tokenizer = NULL;
	l->line_num = 1;
	l->file = malloc(sizeof(char) * (strlen(file) + 1));
	if (l->file == NULL) {
		lexer_cleanup(l);
		return -1;
	}
...
int lexer_next_token(Lexer *l, Token *token) {
	...
	while (state == TokenizerStateComplete || state == TokenizerStateCompleteInComment) {
		...
		if (line)
			free(line);
		line = malloc(sizeof(char) * (1 + strlen(buf)));
		strcpy(line, buf);
		free(buf);
	}

	if (line == NULL) {
		line = malloc((strlen(l->tokenizer->s) + 1) * sizeof(char));
	...
	if (state == TokenizerStateOk) {
		...
		if (line == NULL)
			...
			bool has_tab = false;
			for (int i = 0; i < lsl; i++) {
				if (i < l->tokenizer->pos && l->tokenizer->s[i] == '\t') {
					has_tab = true;
					long_spacer[i] = '\t';
				} else {
					long_spacer[i] = ' ';
			...
			int span_len = snprintf(NULL, 0,
									"%s%s-->%s %s:%llu:%i\n%s %s|\n%llu | %s%s"
									"%s %s|%s%s^\n",
									spacer, CYAN, RESET, l->file, l->line_num, lsl, spacer, CYAN,
									l->line_num, RESET, line, spacer, CYAN, RESET, long_spacer);
			token->span = malloc((span_len + 1) * sizeof(char));
			if (token->span == NULL) {
				free(line);
				return LexerStateErr;
			}
			snprintf(token->span, span_len + 1,
					 "%s%s-->%s %s:%llu:%i\n%s %s|\n%llu | %s%s"
					 "%s %s|%s%s^\n",
					 spacer, CYAN, RESET, l->file, l->line_num, lsl, spacer, CYAN, l->line_num,
					 RESET, line, spacer, CYAN, RESET, long_spacer);
			free(line);
		}
