...
int tokenizer_proc_string_literal(Tokenizer *t, Token *next) {
	next->type = TokenTypeStringLiteral;
	u64 start = t->pos;
	while (true) {
		// if we're at the end of the string, break
		if (t->pos >= t->len)
...
int tokenizer_proc_char_literal(Tokenizer *t, Token *next) {
	next->type = TokenTypeCharLiteral;
	u64 start = t->pos;
	while (true) {
		// if we're at the end of the string, break
		if (t->pos >= t->len)
...
int tokenizer_proc_num_literal(Tokenizer *t, Token *next) {
	...
	while (true) {
		...
		if (t->pos == start) {
			...
			break;
		if (ch == '.') {
			if (next->type == TokenTypeFloatLiteral) {
				// cannot have more than one dot.
				return TokenizerStateErr;
			}
			next->type = TokenTypeFloatLiteral;
		}
		t->pos += 1;
	}
	u64 end = t->pos;
...
int tokenizer_proc_punct(Tokenizer *t, Token *next) {
	...
	// allocate and copy over the token
	next->token = mymalloc(sizeof(char) * 5);
	if (next->token == NULL)
		return TokenizerStateErr;
...
int tokenizer_next_token(Tokenizer *t, Token *next) {
	...
	if ((first >= 'a' && first <= 'z') || (first >= 'A' && first <= 'Z') || first == '_') {
		...
		if (ret == TokenizerStateOk &&
			...
			next->type = TokenTypeBoolLiteral;
		}
		return ret;
	} else if (first == '\"') {
		// string literal
