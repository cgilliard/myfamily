...
MyTest(log, test_unit_tostr) {
	...
	idebug(&log, "uii={},s={},u2={}", UNIT, s2, UNIT);
...
MyTest(log, testlogmacro) {
	...
	Log log =
		...
		ShowTerminal(true), ShowLogLevel(true), ShowTimestamp(true),
		FormatterSize(1500), ShowLineNum(true), ShowMillis(true),
		ShowLineNum(true), ShowColors(true), MaxAgeMillis(100000),
		MaxSizeBytes(4000), LineNumMaxLen(30), FileHeader("myheader"),
		LogFilePath("./bin/testlogmacro.log"));
